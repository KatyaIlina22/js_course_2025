...
// 1) загальну вартість усіх сайтів
let totalSumm:number = siteList.reduce((prevSum,site)=>prevSum+site.siteCost,0)
document.write(`1. загальна вартість усіх сайтів: ${totalSumm}<br>`)
// 2) кількість сайтів, що було зроблено між 2000 та 2009 рр.
let numberOfSites:number = siteList.reduce((prevCount,site)=>site.yearOfRelease>2000&&site.yearOfRelease<2009?prevCount+1:prevCount,0)
document.write(`2. кількість сайтів, що було зроблено між 2000 та 2009 рр.: ${numberOfSites}<br>`)

// 3) кількість сайтів, де сума спонсорських вкладень була більшою за 100000
let numberOfSitesAmountOfSponsorshipInvestmentsWasMoreThan100000:number = 0
siteList.forEach(site => {
    const sum:number = site.sponsors.reduce((prevSum,sponsor)=>sponsor.investmentAmount+prevSum,0)
    if(sum>100000) numberOfSitesAmountOfSponsorshipInvestmentsWasMoreThan100000+=1
});
...
const siteListWithProfit:SiteWithProfit[] = siteList.map(site=>{
   const sumOfInvestmentAmount:number = site.sponsors.reduce((prevSum,sponsor)=>prevSum+sponsor.investmentAmount,0)
const profit:number = site.siteCost-sumOfInvestmentAmount
    return {...site,profit}
})


const maxProfitSite:SiteWithProfit = siteListWithProfit.reduce((prevMax,site)=>site.profit>prevMax.profit?site:prevMax)
...
// 7) Створити 2 окремих списки з копіями об’єктів, що містять сайти з вартість до 10000 і більше 10000
const sitesWithCostLess10000:Site[]=[]
const sitesWithCostMore10000:Site[]=[]
siteList.forEach(site=>{
    if(site.siteCost<10000) sitesWithCostLess10000.push(JSON.parse(JSON.stringify(site)))
        if(site.siteCost>10000) sitesWithCostMore10000.push(JSON.parse(JSON.stringify(site)))
})
