...
class TMoney {
    constructor(dollarExchangeRate = 40) {
        this.amountInDollars = 0;
        this.dollarExchangeRate = 0;
    ...
    getNewDollarExchangeRate() {
        if (this.AmountInDollars <= 0)
            throw new Error("На рахунку 0, неможливо обчислити");
        const newDollarExchangeRate = this.DollarExchangeRate + (100 / this.AmountInDollars);
        return newDollarExchangeRate;
    }
    toString() {
...
function question() {
    ...
    const userMoney = userAnswer !== null ? parseFloat(userAnswer) : NaN;
    return userMoney;
}
