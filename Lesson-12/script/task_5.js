...
if (confirm('Почати тестування?')) {
    ...
    const names = ["Olga", "Oleg", "Anna", "Oleksand", "Alex", "Katia", "Poll", "Olena"];
    ...
    function binarySearch(array, searchEl, min, max) {
        if (min <= max) {
            let middle = Math.floor((min + max) / 2);
            if (array[middle] === searchEl)
                return middle;
            if (array[middle] > searchEl)
                return binarySearch(array, searchEl, min, max = middle - 1);
            if (array[middle] < searchEl)
                return binarySearch(array, searchEl, middle + 1, max);
        }
        return -1;
    }
    if (binarySearch(names, searchElement, 0, names.length - 1) === -1)
        document.write(`Ім'я ${searchElement} не знайдено <br>`);
    else {
